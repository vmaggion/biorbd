...
internal_forces::muscles::HillType::HillType()
    : internal_forces::muscles::Muscle(),
      ...
      m_cste_FvCE_2(
          std::make_shared<utils::Scalar>(
              -.33 / 2 * *m_cste_FvCE_1 / (1 + *m_cste_FvCE_1))),
...
internal_forces::muscles::HillType::DeepCopy() const {
  internal_forces::muscles::HillType copy;
  copy.DeepCopy(*this);
  return copy;
}

void internal_forces::muscles::HillType::DeepCopy(
    const internal_forces::muscles::HillType &other) {
...
void internal_forces::muscles::HillType::computeFlCE(
    const internal_forces::muscles::State &emg) {
  *m_FlCE =
      exp(-pow(
              (position().length() / m_characteristics->optimalLength() /
                   (*m_cste_FlCE_1 * (1 - emg.activation()) + 1) -
               1),
              2) /
          *m_cste_FlCE_2);
}
...
void internal_forces::muscles::HillType::computeFvCE() {
  ...
  *m_FvCE = IF_ELSE_NAMESPACE::if_else(
      ...
      (1.0 - std::fabs(v) / this->characteristics().maxShorteningSpeed()) /
          (1.0 + std::fabs(v) / this->characteristics().maxShorteningSpeed() /
                     *m_cste_FvCE_1),
      (1.0 - 1.33 * v / this->characteristics().maxShorteningSpeed() /
                 *m_cste_FvCE_2) /
          (1 -
           v / this->characteristics().maxShorteningSpeed() / *m_cste_FvCE_2));
#else
  if (v <= 0)
    *m_FvCE = (1 - fabs(v) / this->characteristics().maxShorteningSpeed()) /
              (1 + fabs(v) / this->characteristics().maxShorteningSpeed() /
                       *m_cste_FvCE_1);
  else
    *m_FvCE =
        (1 - 1.33 * v / this->characteristics().maxShorteningSpeed() /
                 *m_cste_FvCE_2) /
...
void internal_forces::muscles::HillType::computeFlPE() {
  ...
  *m_FlPE = IF_ELSE_NAMESPACE::if_else_zero(
      ...
      exp(*m_cste_FlPE_1 *
              (position().length() / characteristics().optimalLength() - 1) -
          *m_cste_FlPE_2));
#else
