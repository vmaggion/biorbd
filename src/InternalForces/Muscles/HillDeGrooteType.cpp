...
internal_forces::muscles::HillDeGrooteType::DeepCopy() const {
...
void internal_forces::muscles::HillDeGrooteType::computeFlPE() {
  utils::Scalar kpe = 4;
  utils::Scalar e0 = 0.6;
  utils::Scalar normLength =
      position().length() / characteristics().optimalLength();
...
void internal_forces::muscles::HillDeGrooteType::computeFvCE() {
  ...
  *m_FvCE = d1 * std::log(
                     (d2 * norm_v + d3) +
                     std::sqrt((d2 * norm_v + d3) * (d2 * norm_v + d3) + 1)) +
            d4;
}
